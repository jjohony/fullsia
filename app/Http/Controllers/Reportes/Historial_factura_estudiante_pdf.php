...
class Historial_factura_estudiante_pdf extends \FPDF
{
    ...
    public function Header() {
        ...
        $this->Cell(185, 6, utf8_decode('HISTORIAL DE PAGOS DEL ESTUDIANTE'), 0, 1, 'C'); 
        $this->Line(55,$this->GetY(),160,$this->GetY());

        $this->Ln(5);

        $this->SetFont('Arial', '', 9);
        $this->Cell(33, 6, utf8_decode('ESTUDIANTE:'), 0, 0, 'L'); 
        $this->Cell(70, 6, utf8_decode($this->factura[0]->nombre." ".$this->factura[0]->paterno." ".$this->factura[0]->materno." "), 0, 1, 'L');
        
        $this->Cell(33, 6, utf8_decode('CI:'), 0, 0, 'L'); 
        $this->Cell(50, 6, utf8_decode(date("d-m-Y",strtotime($this->factura[0]->numero_documento." ".$this->factura[0]->expedido))), 0, 1, 'L');
        $this->Cell(33, 6, utf8_decode('CARRERA:'), 0, 0, 'L'); 
        $this->Cell(70, 6, utf8_decode($this->factura[0]->carrera), 0, 1, 'L');
        $this->Cell(33, 6, utf8_decode('FECHA:'), 0, 0, 'L'); 
        $this->Cell(70, 6, utf8_decode(date("d-m-Y")), 0, 1, 'L');
        

        $this->SetFont('Arial', '', 9);
        $this->Cell(10, 6, utf8_decode('NRO.'), 1, 0, 'C'); 
        $this->Cell(80, 6, utf8_decode("COMCEPTO"), 1, 0, 'C');
        $this->Cell(30, 6, utf8_decode("GESTION"), 1, 0, 'C');
        $this->Cell(30, 6, utf8_decode("FECHA"), 1, 0, 'C');
        $this->Cell(30, 6, utf8_decode("MONTO"), 1, 1, 'C');
    }
    ...
    {   
        ...
        foreach ($factura as $key => $value) {
            $this->SetFont('Arial', '', 9);
            $this->Cell(10, 6, utf8_decode($key+1), 1, 0, 'C'); 
            $this->Cell(80, 6, utf8_decode($value->concepto_pago), 1, 0, 'C');
            $this->Cell(30, 6, utf8_decode($value->gestion), 1, 0, 'C');
            $this->Cell(30, 6, utf8_decode($value->fecha), 1, 0, 'C');
            $this->Cell(30, 6, utf8_decode($value->monto), 1, 1, 'C');            
            $total=$total+$value->monto;
        }
        //}
        
        $this->Cell(150, 6, utf8_decode("TOTAL"), 1, 0, 'C'); 
