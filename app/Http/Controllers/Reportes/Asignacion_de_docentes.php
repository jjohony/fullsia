...
class Asignacion_de_docentes extends \FPDF
{
    ...
    public function reporte($asignacion)
    {   
        ...
        $this->Cell(185, 6, utf8_decode('ASIGNACIÓN DE DOCENTES'), 0, 1, 'C'); 
        $this->SetFont('Arial', '', 10);

        $gestion=($_GET["gestion"]!="")?$_GET["gestion"]:"";
        $periodo_gestion=($_GET["periodo_gestion"]!="")?$_GET["periodo_gestion"]:"";
    
        
        $this->Cell(185, 6, utf8_decode("GESTIÓN ".$periodo_gestion." ".$gestion), 0, 1, 'C'); 
        ...
        {   
            ...
            foreach ($asignacion as $key => $value) {
                ...
                if($numero_documento!=$value->numero_documento)
                {
                    $this->Ln(5);
                    $this->Cell(185, 6, utf8_decode($value->grado_academico." ".$value->nombre." ".$value->paterno." ".$value->materno), 0, 1, 'C');
                    $this->Line(15,$this->GetY(),205,$this->GetY());
                    $this->Cell(10, 6, utf8_decode("Nro. "), 0, 0, 'C');
                    $this->Cell(90, 6, utf8_decode("Materia(s)"), 0, 0, 'C');
                    $this->Cell(25, 6, utf8_decode("Sigla"), 0, 0, 'C');
                    $this->Cell(25, 6, utf8_decode("Semestre"), 0, 0, 'C');
                    $this->Cell(20, 6, utf8_decode("Plan"), 0, 0, 'C');
                    $this->Cell(20, 6, utf8_decode("Carrera"), 0, 1, 'C');
                    $cont=0;
                    $numero_documento=$value->numero_documento;   

                }
                $cont++;
                $this->Cell(10, 6, utf8_decode($cont), 0, 0, 'C');
                $this->AjustaCelda(90, 6, utf8_decode($value->asignatura), 0, 0, 'C');
                $this->Cell(25, 6, utf8_decode($value->sigla_asignatura), 0, 0, 'C');
                $this->Cell(25, 6, utf8_decode(Funciones::numero_cardinal($value->nivel_asignatura)), 0, 0, 'C');
                $this->Cell(20, 6, utf8_decode($value->plan_pensum), 0, 0, 'C');
                $this->Cell(20, 6, utf8_decode($value->sigla_carrera), 0, 1, 'C');
