...
class Boleta_de_asignacion_de_materias extends PDF
{
    ...
    public function reporte($estudiante)
    {   
        if(empty($estudiante))
        {
            return ;
        ...
        for ($i=0.1; $i < 2; $i=$i+0.1) { 
            $this->Line(15,15+$i,200,15+$i);

            $this->Line(15+$i,15,15+$i,80);

            $this->Line(200+$i,15,200+$i,80);

            $this->Line(15,80+$i,202,80+$i);

            $this->Line(107.5+$i,15,107.5+$i,80);
        ...
        $this->RoundedRect(22, 22, 30, 30, 3);


        if($this->estudiante->foto_imagen!="")
        {
            $this->Image('../public/img/'.$this->estudiante->foto_imagen, 23, 23, 28, 28);
        }
        else
        {
            $this->Image('../public/assets/img/persona.png', 23, 23, 28, 28);    
        }
        ...
        $this->Cell(30, 6, utf8_decode($usuario->name), 0, 1, 'C');  

        $this->SetX(22);
        $this->Cell(30, 6, utf8_decode($this->estudiante->numero_documento." ".$this->estudiante->expedido), 0, 1, 'C');  
        
        $this->SetY(28);
        $this->SetX(60);


        $this->Cell(30, 6, utf8_decode($this->estudiante->nombre), 0, 1, 'C');  

        $this->SetX(60);
        $this->Cell(30, 6, utf8_decode($this->estudiante->paterno." ".$this->estudiante->materno), 0, 1, 'C');  

        $this->SetX(55);

        $this->SetFont("Arial","",8);
        $this->AjustaCelda(45, 6, utf8_decode($this->estudiante->carrera), 0, 1, 'C');          
        ...
        $this->SetX(55);
        $this->AjustaCelda(45, 7, utf8_decode(date("d/m/Y",strtotime($this->estudiante->fecha))), 0, 1, 'R');


        $periodo_gestion=($_GET["periodo_gestion"]!="ANUAL")?$_GET["periodo_gestion"]:"";
        $gestion=$_GET["gestion"];
        $this->SetX(55);
        $this->AjustaCelda(45, 7, utf8_decode($periodo_gestion." ".$gestion), 0, 1, 'R');

        $this->SetY(28);
        $this->SetX(105);
        $this->Rotate(270);
        $this->AjustaCelda(45, 5, utf8_decode($usuario->name), 0, 1, 'C');

        $this->Rotate(0);
        $this->RoundedRect(115, 22, 70, 50, 3);


        $this->SetY(25);
        $this->SetX(117);
        $this->MultiCell(64, 5, utf8_decode($this->configuracion->informacion_instituto));
        ...
        $this->AjustaCelda(155, 7, utf8_decode($periodo_gestion." ".$gestion), 0, 0, 'C');
        $this->AjustaCelda(30, 7, utf8_decode("La Paz, ".date("d/m/Y")), 0, 1, 'C');

        $this->AjustaCelda(155, 7, utf8_decode($this->estudiante->carrera), 0, 0, 'C');
        $this->AjustaCelda(30, 7, utf8_decode($this->estudiante->plan_pensum), 0, 1, 'C');

        $this->AjustaCelda(155, 7, utf8_decode($this->estudiante->paterno." ".$this->estudiante->materno." ".$this->estudiante->nombre), 0, 0, 'C');
        $this->AjustaCelda(30, 7, utf8_decode($usuario->name), 0, 1, 'C');

        $modalidad=($estudiante[0]->tipo_gestion=="semestral")?"SEMESTRE":"AÑO";
        
        $this->SetFont("Arial","B",10);
        $this->AjustaCelda(8, 7, utf8_decode("N°"), 1, 0, 'C');

        $this->AjustaCelda(30, 7, utf8_decode($modalidad), 1, 0, 'C');
        $this->AjustaCelda(30, 7, utf8_decode("SIGLA"), 1, 0, 'C');

        $this->AjustaCelda(117, 7, utf8_decode("MATERIA(S)"), 1, 1, 'C');

        $cont=0;
        $this->SetFont("Arial","",10);
        foreach ($estudiante as $key => $value) {
            $cont++;
            $this->AjustaCelda(8, 7, utf8_decode($cont), 1, 0, 'C');
            $this->AjustaCelda(30, 7, utf8_decode(Funciones::numero_cardinal($value->nivel_asignatura)), 1, 0, 'C');
            $this->AjustaCelda(30, 7, utf8_decode($value->sigla_asignatura), 1, 0, 'C');

            $this->AjustaCelda(117, 7, utf8_decode($value->asignatura), 1, 1, 'L');            
        }
