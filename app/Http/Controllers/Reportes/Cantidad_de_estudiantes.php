...
class Cantidad_de_estudiantes extends \FPDF
{
    ...
    public function Header() {
        ...
        $this->Cell(185, 6, utf8_decode("CANTIDAD DE ESTUDIANTES POR ASIGNATURA"), 0, 1, 'C'); 
    ...
    {     
        ...
        $this->Cell(20, 6, utf8_decode("Sigla"), 1, 0, 'C');
        $this->Cell(75, 6, utf8_decode("Asignatura(s)"), 1, 0, 'C');
        $this->Cell(25, 6, utf8_decode("Estudiantes"), 1, 0, 'C');
        $this->Cell(30, 6, utf8_decode("Cantidad"), 1, 0, 'C');
        $this->Cell(30, 6, utf8_decode("Total estudiantes"), 1, 1, 'C');
    }
    ...
    public function reporte($estudiante,$carrera)
    {   
        ...
        {   
            ...
            $cont++;
                $this->SetFont('Arial', '', 9);
                if($semestre!=$value->nivel_asignatura)
                {
                    if($swich==1)
                    {
                        $this->Ln(5);    
                    ...
                    $swich=1;
                    switch ($value->modalidad_asignatura) {
                        case 'semestral':
                            $modalidad="SEMESTRAL";
                    ...
                    $this->Cell(190, 5, utf8_decode(Funciones::numero_cardinal($value->nivel_asignatura)." ".$modalidad_aux), 1, 1, 'C');
                    $semestre=$value->nivel_asignatura;
                    $this->cabecera2();
                }
                ...
                $this->Cell(10, 10, utf8_decode($cont), 1, 0, 'C',$boolean);
                $this->AjustaCelda(20, 10, utf8_decode($value->sigla_asignatura), 1, 0, 'C',$boolean);
                $this->AjustaCelda(75, 10, utf8_decode($value->asignatura), 1, 0, 'C',$boolean);
                $x=$this->GetX();
                $y=$this->GetY();
                $this->Cell(25, 5, utf8_decode("Varones"), 1, 0, 'C',$boolean);
                
                $this->Cell(30, 5, utf8_decode($varones), 1, 1, 'C',$boolean);
                $this->SetX($x);
                $this->Cell(25, 5, utf8_decode("Mujeres"), 1, 0, 'C',$boolean);
                $this->Cell(30, 5, utf8_decode($mujeres), 1, 0, 'C',$boolean);
                $x=$this->GetX();
                $this->SetY($y);
                $this->SetX($x);
                $this->SetFont('Arial', '', 15);
                $this->Cell(30, 10, utf8_decode($value->total), 1, 1, 'C',$boolean);
