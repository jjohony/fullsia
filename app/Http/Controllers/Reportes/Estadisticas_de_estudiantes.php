...
class Estadisticas_de_estudiantes extends \FPDF
{
    ...
    public function Header() {
        ...
        $this->Cell(185, 6, utf8_decode($this->configuracion->nombre), 0, 1, 'C'); 
        $this->SetFont('Arial', '', 10);

        $this->Ln(5);
        $this->SetFont('Arial', '', 9);        
        $this->Cell(185, 6, utf8_decode("Estadisticas de Estudiantes"), 0, 1, 'C'); 
        $gestion=$_GET["gestion"];
        $periodo_gestion=($_GET["periodo_gestion"]=="anual")?"":$_GET["periodo_gestion"];
        $this->Cell(140, 6, utf8_decode("CARRERA: ".$this->carrera->carrera), 0, 0, 'L'); 
        $this->Cell(45, 6, utf8_decode($periodo_gestion." ".$gestion), 0, 1, 'R'); 
    ...
    {   
        ...
        $varones=($estudiante[0]->varones!="")?$estudiante[0]->varones:0;
        $mujeres=($estudiante[0]->mujeres!="")?$estudiante[0]->mujeres:0;

        $varones_porcentaje=($varones!=0&&$estudiante[0]->total!=0)?$varones*100/$estudiante[0]->total:0;
        $casilla_porcentaje=($varones!=0&&$estudiante[0]->total!=0)?$varones*100/$estudiante[0]->total:0.1;

        $boolean=true;
        $this->Cell(45+80+64, 5, utf8_decode("ESTADISTICA GENERAL"), 1, 1, 'C');

        $this->cabecera2();
        
        $y=$this->GetY();
        $this->Cell(45, 5, utf8_decode("Varones"), 1, 0, 'R');
        $this->SetFillColor(39, 158, 230); 
        $this->Cell($casilla_porcentaje*0.65, 5, utf8_decode(""), 1, 0, 'R',$boolean);
        $this->Cell(80-$casilla_porcentaje*0.65, 5, utf8_decode(number_format($varones_porcentaje,2)." %"), 1, 0, 'C');
        $this->Cell(32, 5, utf8_decode($varones." estudiante(s)"), 1, 0, 'C');
        
        $this->Ln();

        $mujeres_porcentaje=($mujeres!=0&&$estudiante[0]->total!=0)?$mujeres*100/$estudiante[0]->total:0;
        $casilla_porcentaje=($mujeres!=0&&$estudiante[0]->total!=0)?$mujeres*100/$estudiante[0]->total:0.1;

        $this->SetFillColor(228, 76, 170); 
        $this->Cell(45, 5, utf8_decode("Mujeres"), 1, 0, 'R');
        $this->Cell($casilla_porcentaje*0.65, 5, utf8_decode(""), 1, 0, 'C',$boolean);
        $this->Cell(80-$casilla_porcentaje*0.65, 5, utf8_decode(number_format($mujeres_porcentaje,2)." %"), 1, 0, 'C');
        
        $this->Cell(32, 5, utf8_decode($mujeres." estudiante(s)"), 1, 0, 'C');
        $x=$this->GetX();
        $this->SetY($y);
        $this->SetX($x);
        $this->SetFont('Arial', '', 15);
        $this->Cell(32, 10, utf8_decode($estudiante[0]->total), 1, 1, 'C');

        $this->SetFont('Arial', '', 9);

        $this->SetFillColor(45, 158, 44); 
        $this->Cell(45, 5, utf8_decode("TOTAL"), 1, 0, 'R');
        $this->Cell(65, 5, utf8_decode(""), 1, 0, 'R',$boolean);
        $this->Cell(15, 5, utf8_decode("100 %"), 1, 0, 'R');
        $this->Cell(32, 5, utf8_decode($estudiante[0]->total." estudiante(s)"), 1, 1, 'C');
    ...
    {     
        ...
        $this->Cell(45, 6, utf8_decode("Estudiantes"), 1, 0, 'C');
        $this->Cell(80, 6, utf8_decode("Porcentajes"), 1, 0, 'C');
        $this->Cell(32, 6, utf8_decode("Cantidad"), 1, 0, 'C');
        $this->Cell(32, 6, utf8_decode("Total estudiantes"), 1, 1, 'C');
    }
    ...
    public function reporte($estudiante,$estudiante_detalle,$carrera)
    {   
        ...
        {   
            ...
            $this->SetDrawColor(0, 0,0 );
            foreach ($estudiante_detalle as $key => $value) {
                $cont++;
                $this->SetFont('Arial', '', 9);
                
                //var_dump($key);die;
                $varones=($value->varones!="")?$value->varones:0;
                $mujeres=($value->mujeres!="")?$value->mujeres:0;

                $varones_porcentaje=($varones!=0&&$value->total!=0)?$varones*100/$value->total:0;
                $casilla_porcentaje=($varones!=0&&$value->total!=0)?$varones*100/$value->total:0.1;

                $boolean=true;

                $this->Cell(45+80+64, 5, utf8_decode(Funciones::numero_cardinal($key)." ".$modalidad_aux), 1, 1, 'C');
                ...
                $mujeres_porcentaje=($mujeres!=0&&$value->total!=0)?$mujeres*100/$value->total:0;
                $casilla_porcentaje=($mujeres!=0&&$value->total!=0)?$mujeres*100/$value->total:0.1;
                ...
                $this->SetX($x);
                $this->SetFont('Arial', '', 15);
                $this->Cell(32, 10, utf8_decode($value->total), 1, 1, 'C');
                ...
                $this->Cell(65, 5, utf8_decode(""), 1, 0, 'R',$boolean);
                $this->Cell(15, 5, utf8_decode("100 %"), 1, 0, 'R');
                $this->Cell(32, 5, utf8_decode($value->total." estudiante(s)"), 1, 1, 'C');
                $this->Ln(5);
