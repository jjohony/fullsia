...
class Asistencia_de_estudiantes extends \FPDF
{
    ...
    {   
        ...
        $this->Cell(0, 3, utf8_decode($this->configuracion->nombre), 0, 0, 'C');  
        $this->Ln(7);
        $this->SetFont('Arial', 'I', 9);   //TAMAÃ‘O DE LETRA     
        $this->Cell(0, 5, 'R.M. Nro.', 0, 0, 'C');
        $this->Ln(3);
        $this->Cell(0, 0, '', 0, 0, 'C');       
        $this->SetFont('Arial', '', $this->tamLetra);
        $this->SetFont('Arial', 'B', 15);
        $this->Ln(8);
        $this->Cell(0,0,'CONTROL DE ASISTENCIA DE ESTUDIANTES',0,1,'C');
        $this->Ln(7);
        $this->SetFont('Arial', '', 13);
        $this->Cell(0,0,date("d / m / Y",strtotime($fecha)),0,0,'C');
    ...
    {     
        ...
        $this->Cell(90,5,utf8_decode('PARALELO:  "'.$curso->paralelo_curso.'"'),0,0,'L');

        $modalidad=($curso->modalidad_asignatura=="semestral")?"SEMESTRE":"AÑO";

        $this->Cell(60,5,utf8_decode($modalidad.': '.Funciones::numero_cardinal($curso->nivel_asignatura)),0,1,'L');
        ...
        $this->Cell(90,5,utf8_decode('DOCENTE: '.$curso->grado_academico." ".$curso->nombre." ".$curso->paterno." ".$curso->materno),0,1,'L');
    ...
    {     
        ...
        $this->SetFont('Arial', '', 9);
        $this->Cell(9,6,utf8_decode('N°'),1,0,'C');
        $this->SetFont('Arial', '', 9);
        $this->Cell(153,6,utf8_decode('APELLIDOS Y NOMBRES'),1,0,'C');        
        $this->Cell(30,6,utf8_decode('ASISTENCIA'),1,1,'C');        
    ...
    public function reporte($curso,$fechas,$estudiante)
    {   
            ...
            { 
                ...
                if(!empty($estudiante))
                {
                    ...
                    foreach ($estudiante as $key1 => $value1) {
                        ...
                        $this->SetFont('Arial', '', 9);
                        $this->Cell(9,5,$cont,1,0,'C');            

                        $this->Cell(153,5,utf8_decode(strtoupper($value1->paterno." ".$value1->materno." ".$value1->nombre)),1,0,'L');        
                        $asistencia=estudiante::reporte_asistencia_de_estudiantes_asistencia($curso->id_curso,$value->fecha,$value1->id_estudiante);
                        if(!empty($asistencia))
                        {
                            $asistencia="A";
                        }
                        ...
                        $this->Cell(30,5,utf8_decode("".$asistencia),1,1,'C');        
                    ...
                    else
                    {
                        ...
                        {
                            $this->Cell(9,5,'',1,0,'L');            
                            $this->Cell(153,5,'',1,0,'L');            
                            $this->AjustaCelda(30,5,'',1,1,'L');
    ...
    public function pieActa($aprovados,$reprovados,$nsp,$cont,$curso)
    {
        ...
        $fecha_literal=$this->fecha_literal(date("Y-m-d"));
