...
class Detalle_de_pagos_por_estudiante extends \FPDF
{
    ...
    {   
        ...
        $this->Cell(185, 6, utf8_decode('DETALLE DE PAGOS POR ESTUDIANTE'), 0, 1, 'C'); 
        ...
        {   
            ...
            foreach ($factura as $key => $value) {
                ...
                if($carrera!=$value->carrera)
                {
                    ...
                    $this->Cell(55, 6, utf8_decode("ESTUDIANTE"), 0, 0, 'C');
                    $this->Cell(45, 6, utf8_decode("CARRERA"), 0, 0, 'C');
                    $this->Cell(40, 6, utf8_decode("CONCEPTO"), 0, 0, 'C');
                    $this->Cell(20, 6, utf8_decode("SUB TOTAL"), 0, 0, 'C');
                    $this->Cell(20, 6, utf8_decode("TOTAL (Bs)"), 0, 1, 'C');
                ...
                $this->Line(15,$this->GetY(),205,$this->GetY());
                $detalle=factura_estudiante::reporte_detalle_de_pagos_por_estudiante_detalle($value->id_carrera,$value->id_estudiante,$_GET["fecha_inicio"],$_GET["fecha_fin"]);
                $sub_total=0;
                $this->Cell(10, 6, utf8_decode($key+1), 0, 0, 'C');
                $this->Cell(55, 6, utf8_decode($value->paterno." ".$value->materno." ".$value->nombre), 0, 0, 'L');
                $this->Cell(45, 6, utf8_decode($value->carrera), 0, 0, 'C');
                $y=$this->GetY();
                foreach ($detalle as $key1 => $value1) {

                    $this->SetX(80+45);
                    
                    $this->Cell(40, 6, utf8_decode($value1->concepto_pago), 0, 0, 'C');
                    $this->Cell(20, 6, utf8_decode($value1->monto), 0, 1, 'C');
                    $sub_total=$value1->monto+$sub_total;    

                }
                $y1=$this->GetY();
                
                $this->SetY($y);
                $this->SetX(80+45+40+20);
                $this->Cell(20, 6, utf8_decode($sub_total), 0, 1, 'C');
                $this->SetY($y1);
                $this->Line(15,$this->GetY(),205,$this->GetY());
                $total=$sub_total+$total;

                if(isset($factura[$key+1]))
                {
                    if($factura[$key+1]->carrera!=$carrera)
                    {
                        $this->Cell(170, 6, utf8_decode("TOTAL"), 0, 0, 'L');
                        $this->Cell(20, 6, utf8_decode($total), 0, 1, 'C');        
                    }
