       ...
       <div class="panel panel-inverse">
         ...
         <table id="data-table" class="table table-striped table-bordered">
            ...
            <tbody id="tbody-curso">
              {{csrf_field()}}
              <?php if(!empty($notificacion)){$i=1; foreach ($notificacion as $key=>$value) {
                  
                  
               ?>
                <tr id="tr_{{$value->id}}">
                  <td>{{$key+1}}</td>
                  <td>{{$value->notificacion_docente}}</td>
                  <td><?php echo date("d-m-Y  ",strtotime($value->created_at));?></td>
                  <td><?php echo date("d-m-Y  ",strtotime($value->updated_at));?></td>
                  <td>
                    <a href="{{url('')}}/docente/notificacion/{{$value->id}}/show" title="" class="btn btn-success" >
                         Ver
                    </a>
